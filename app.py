...
def render_tab(tab_df, tab_label, valid_cols):
    ...
    with col_set1:
        ...
        # 💡 모바일 최적화 스위치 추가 (탭마다 독립적인 key 부여)
        st.markdown("---")
        is_mobile = st.toggle("📱 모바일 화면 최적화 (점/글씨 크기 축소)", key=f"mob_{tab_label}")
    ...
    with col_set2:
        ...
        fig.update_traces(
            ...
            textfont_size=font_size,                 # 글씨 크기 연동
            marker=dict(size=marker_size, line=dict(width=1 if is_mobile else 2, color='white')) # 점 크기 연동
        )
        fig.update_layout(
            height=chart_height,                     # 높이 연동
