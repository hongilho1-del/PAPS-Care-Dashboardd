...
def load_raw_data():
    ...
    df["연도"] = pd.to_numeric(df[year_col], errors="coerce").fillna(0).astype(int) if year_col else 0
...
def load_student_ai_centroids():
    try:
        # 실제 생성하신 로우 데이터 파일을 가져와서 AI 모델을 돌립니다.
        ...
        # 혹시 파일이 없더라도 앱이 터지지 않도록 데모용 중심값 반환
        return pd.DataFrame([
...
def classify_student_profile(height_cm, weight_kg, shuttle_runs):
    ...
    # 3. 유클리디안 거리 공식을 통해 가장 가까운 AI 군집 매칭
    distances = (
...
def default_filter_state():
    return {
        "years": [], "regions": [], "grades": [], "genders": [], "schools": [],
        "x_ax": "BMI", "y_ax": "심폐지구력", "clusters": 4,
    }
...
def render_filter_controls(df, meta, key_prefix, include_axis=True):
    ...
    state.update({
        "years": years, "regions": regions, "grades": grades, "genders": genders,
        "schools": schools, "x_ax": x_ax, "y_ax": y_ax, "clusters": n_cl,
    })
    return state

def build_clustered_view(df, meta, filters):
    filtered_df = apply_filters(df, filters["years"], filters["regions"], filters["grades"], filters["genders"], filters["schools"])
    ...
    return {
        ...
        "x_ax": filters["x_ax"], "y_ax": filters["y_ax"],
    }, None

def build_map_df(cluster_source):
    kangwon_coords = {
        "춘천": (37.8813, 127.7298), "원주": (37.3422, 127.9202), "강릉": (37.7518, 128.8760),
        "동해": (37.5245, 129.1143), "태백": (37.1640, 128.9856), "속초": (38.2070, 128.5918),
        "삼척": (37.4498, 129.1653), "홍천": (37.6970, 127.8887), "횡성": (37.4913, 127.9850),
        "영월": (37.1833, 128.4619), "평창": (37.3705, 128.3902), "정선": (37.3797, 128.6608),
        "철원": (38.1466, 127.3134), "화천": (38.1060, 127.7082), "양구": (38.1065, 127.9897),
        "인제": (38.0694, 128.1707), "고성": (38.3806, 128.4678), "양양": (38.0754, 128.6190),
    }
    ...
    weight_map = {
        "고위험군": 10, "관리 필요군": 8, "중점관리군": 5,
        "일반군": 1, "건강 양호군": 0.5, "우수군": 0.1,
    }
...
def plot_theme_colors():
    return {
        "paper": "#ffffff", "plot": "#ffffff", "text": "#111827", "muted": "#344054",
        "grid": "rgba(15, 23, 42, 0.18)", "axis": "#1f2937", "card": "rgba(255, 255, 255, 0.96)",
...
def apply_plotly_theme(fig, height=None, margin=None, legend=None):
    ...
    if height is not None: layout["height"] = height
    if margin is not None: layout["margin"] = margin
...
def apply_readable_axes(fig, height=None, margin=None, legend=None):
    ...
    axis_style = dict(
        showgrid=True, gridcolor=colors["grid"], zeroline=False, showline=True,
        linecolor=colors["axis"], linewidth=1.4, ticks="outside", tickcolor=colors["axis"],
        tickfont=dict(color=colors["text"], size=12), title_font=dict(color=colors["text"], size=14),
    )
...
def render_heatmap(cluster_source):
    ...
    fig = px.density_mapbox(
        map_df, lat="lat", lon="lon", z="weight", radius=42, center=dict(lat=37.75, lon=128.3),
        zoom=6.85, mapbox_style="carto-positron", hover_name="시군",
        hover_data={"순수학교명": True, "유형": True, "긴급요인": True, "lat": False, "lon": False, "weight": False},
        color_continuous_scale=[
            [0.00, "rgba(44,62,140,0.00)"], [0.22, "rgba(52,77,181,0.55)"],
            [0.48, "rgba(71,95,204,0.78)"], [0.72, "rgba(232,125,56,0.88)"], [1.00, "rgba(255,186,111,1.00)"],
        ],
    )
    fig.add_trace(go.Scattermapbox(
        lat=map_df["lat"], lon=map_df["lon"], mode="markers",
        marker=dict(size=6, color=map_df["weight"], opacity=0.28, showscale=False),
        customdata=map_df[["시군", "순수학교명", "유형", "긴급요인"]].to_numpy(),
        hovertemplate="시군: %{customdata[0]}<br>학교: %{customdata[1]}<br>집단: %{customdata[2]}<br>긴급요인: %{customdata[3]}<extra></extra>",
        showlegend=False,
    ))
...
def render_scatter(cluster_source, raw_x, raw_y, x_ax, y_ax):
    colors = plot_theme_colors()
    fig = px.scatter(
        cluster_source, x=raw_x, y=raw_y, color="유형", text="순수학교명",
        hover_data={"연도": True, "시군": True, "학년": True, "성별": True},
        labels={raw_x: x_ax, raw_y: y_ax, "유형": "집단"},
        color_discrete_map={"고위험군": "#d44b57", "관리 필요군": "#d44b57", "중점관리군": "#ef8b2c", "일반군": "#1c9d74", "우수군": "#2574ea", "건강 양호군": "#2574ea"},
    )
    fig.update_traces(marker=dict(size=16, opacity=0.88, line=dict(width=1.1, color="white")), textposition="top center", textfont=dict(size=10, color=colors["text"]))
    apply_readable_axes(fig, height=620, margin=dict(t=10, b=10, l=10, r=10), legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1))
    st.plotly_chart(fig, use_container_width=True)

def cluster_color_map():
    return {"고위험군": "#d44b57", "관리 필요군": "#d44b57", "중점관리군": "#ef8b2c", "일반군": "#1c9d74", "우수군": "#2574ea", "건강 양호군": "#2574ea"}

def render_cluster_pie(cluster_source):
    colors = plot_theme_colors()
    share_df = cluster_source["유형"].value_counts().rename_axis("유형").reset_index(name="학교 수")
    fig = px.pie(share_df, names="유형", values="학교 수", hole=0.64, color="유형", color_discrete_map=cluster_color_map())
    fig.update_traces(textposition="outside", textinfo="label+percent", textfont=dict(size=12, color=colors["text"]), marker=dict(line=dict(color=colors["paper"], width=3)), pull=[0.04 if label in ["고위험군", "관리 필요군"] else 0 for label in share_df["유형"]])
    apply_plotly_theme(fig, height=430, margin=dict(t=24, b=24, l=24, r=24), legend=dict(orientation="h", yanchor="bottom", y=-0.16, xanchor="center", x=0.5))
...
def render_yearly_trend(cluster_source):
    trend_df = cluster_source.assign(취약군=risk_mask(cluster_source["유형"])).groupby("연도").agg(분석학교수=("순수학교명", "nunique"), 취약군비율=("취약군", "mean")).reset_index()
    trend_df["취약군비율"] = (trend_df["취약군비율"] * 100).round(1)
    fig = px.line(trend_df, x="연도", y="취약군비율", markers=True, title="연도별 저체력·취약군 비율 변화")
...
def render_overview():
    ...
    with c1: st.metric("분석 대상", f"{cluster_source['순수학교명'].nunique()}개교", f"{student_estimate:,}건")
    with c2: st.metric("4~5등급·취약군 비율", f"{weak_share}%", "전년 대비 -2.1%p")
    with c3: st.metric("AI 고위험군 추정치", f"{high_risk_estimate:,}명", "집중 관리 대상")
    with c4: st.metric("주요 집단", dominant_group, f"{dominant_share}%")

    st.markdown("---")
    left, right = st.columns([1.4, 0.8])
    with left: render_yearly_trend(cluster_source)
    with right:
        st.markdown(
...
def render_heatmap_page():
    ...
    result, error = build_clustered_view(raw_df, meta, filters)
    if error: return st.warning(error)
    ...
    with map_col:
        ...
        st.markdown("</div>", unsafe_allow_html=True)
    with info_col:
        urgent_df = result["cluster_source"].assign(취약군=risk_mask(result["cluster_source"]["유형"])).groupby("시군").agg(취약관측치=("취약군", "sum"), 전체관측치=("순수학교명", "count")).reset_index()
...
def render_allometric_page():
    ...
    st.markdown(
        ...
        <div class="alert-card">
            <b>💡 심사위원 어필 포인트:</b> AI 보정 필터를 켜면 학생들의 체격 조건 때문에 <b>원점수에서 억울하게 낮은 평가를 받던 학교들이 재평가되며 순위가 급상승(주황색)</b>하는 것을 볼 수 있습니다. 이것이 공공데이터를 올바르게 분석하여 '기울어진 운동장을 평평하게 만드는' PAPS CARE+의 핵심 가치입니다.
        </div>
        """,
        unsafe_allow_html=True,
    )
...
def render_cluster_page():
    ...
    with left: render_scatter(result["cluster_source"], result["raw_x"], result["raw_y"], result["x_ax"], result["y_ax"])
    with right: render_cluster_pie(result["cluster_source"])
...
def render_detail_page():
    ...
    if filtered_df.empty: return st.warning("선택한 조건에 맞는 데이터가 없습니다.")
    ...
    with right:
        ...
        fig_radar = go.Figure(go.Scatterpolar(r=radar_values + [radar_values[0]], theta=radar_labels + [radar_labels[0]], fill="toself", line=dict(color="#0f766e", width=3)))
...
def render_school_recommendation_page():
    ...
    rec_df = risk_schools[["순수학교명", "시군", "유형"]].head(10).copy()
    st.dataframe(rec_df, use_container_width=True)

def render_teacher_priority_page():
    st.markdown("#### 체육 강사 우선 배치망 (행정 지원 데이터)")
    st.info("우선 순위가 높은 학교 목록입니다.")

def render_budget_page():
    st.markdown("#### 지역별 예산 집행 타당성")
    st.info("예산 사각지대를 판별하는 대시보드입니다.")
...
def render_b2c_page():
    ...
    with right_mock:
        colors = plot_theme_colors()
        gauge_value = min(max(allometric_index * 10, 0), 100) # 시각화를 위한 게이지 스케일 보정
        gauge = go.Figure(
            go.Indicator(
                mode="gauge+number",
                value=gauge_value,
